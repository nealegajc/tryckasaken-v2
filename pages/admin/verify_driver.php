...
if (!isset($_GET['id']) || !isset($_GET['action'])) {
    header('Location: driver_verification.php?error=missing_params');
    exit;
}
...
if (!in_array($action, ['approve', 'reject'])) {
    header('Location: driver_verification.php?error=invalid_action');
    exit;
}
...
if (!$driver) {
    header('Location: driver_verification.php?error=driver_not_found');
    exit;
}
...
if ($updateStmt->execute()) {
    ...
    header("Location: driver_verification.php?success=" . urlencode($message) . "&type=" . $messageType);
} else {
    header('Location: driver_verification.php?error=update_failed');
}
