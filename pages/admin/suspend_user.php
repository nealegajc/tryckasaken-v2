...
if (!isset($_GET['id'])) {
    header('Location: admin.php?error=missing_user_id');
    exit;
}
...
if ($stmt->execute()) {
    ...
    header("Location: view_user.php?id={$userId}&success=" . urlencode($message) . "&type=" . $messageType);
    exit;
} else {
    $error = $conn->error;
    $stmt->close();
    ...
    header("Location: view_user.php?id={$userId}&error=Failed to update status: " . urlencode($error));
    exit;
}
?>
